...
**Autor:** Zaqueu Ribeiro da Costa

**Qualificação:** Pesquisador independente
...
com determinação dos pontos inteiros e aplicação de condições de congruência. O artigo contém um certificado computacional reproduzível em SageMath. Os problemas abertos discutidos no texto incluem extensões para outras interseções, mecanismos de propagação de dados locais e comparação de estruturas ciclotômicas.
...
O arquivo `CHECKSUMS.sha256` registra os hashes SHA-256 do fonte e do PDF fornecidos pelo autor. O conteúdo desses arquivos foi preservado integralmente; apenas seus nomes foram padronizados. Os hashes foram gerados a partir do PDF e do .tex originais e servem como verificação de integridade do arquivo arquivado.

Na preparação deste repositório, o PDF fornecido foi aberto e suas nove páginas foram renderizadas. Não foi realizada recompilação local, pois `pdflatex` não estava instalado no ambiente de execução do repositório. O material preservado e os scripts de verificação são o conteúdo objetivo deste projeto.

## Licença

- O código, os scripts em `scripts/` e os materiais executáveis deste repositório estão licenciados sob a [MIT License](LICENSE).
- O artigo, a prova matemática, a documentação, as imagens e os demais materiais de autoria intelectual em `paper/` e `figures/` estão licenciados sob a [Creative Commons Attribution 4.0 International (CC BY 4.0)](https://creativecommons.org/licenses/by/4.0/).
- A atribuição deve permanecer para: **Zaqueu Ribeiro da Costa**, autor e pesquisador independente.
- Qualquer reutilização, reprodução, adaptação ou citação deve manter a menção ao autor e ao trabalho original.
- A licença do repositório no GitHub foi definida como MIT para compatibilidade com a interface do GitHub; a atribuição do texto acadêmico e da arte permanece sob CC BY 4.0.

Todos os direitos reservados ao autor, Zaqueu Ribeiro da Costa, pesquisador independente.
